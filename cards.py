...
class Hand:
    ...
    def __init__(self, cards):
        ...
        self.__amounts_of_card_values = dict.fromkeys(CardValue, 0)
        for card in cards:
            self.__amounts_of_card_values[card.value] += 1
    ...
    def __get_royal_flush_rank(self):
        if self.__get_straight_flush_rank() is None or self.__cards[0].value != CardValue.ACE:
            return None
        return []
    ...
    def __get_straight_flush_rank(self):
        if self.__get_straight_rank() is None or self.__get_flush_rank() is None:
            return None
        return [self.__cards[0].value]

    def __get_four_of_a_kind_rank(self):
        rank = []
        for value, amount in self.__amounts_of_card_values.items():
            if amount == 4:
                rank.append(value)
                break
        if not rank: 
        ...
        for i, hand in enumerate(self.__HAND_RANKING):
            ...
            if not rank is None:
                return (
                    ...
                    [len(self.__HAND_RANKING) - i - 1] + [card_value.value[0] for card_value in rank]
                )
